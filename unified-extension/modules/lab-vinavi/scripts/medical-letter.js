...
(function() {
  ...
  async function loadDoctorData() {
    try {
      ...
      await fetchDoctorFromVinavi();
      
      // Also check chrome.storage as fallback
      const result = await chrome.storage.local.get(['vinavi_doctor_name', 'vinavi_doctor_code', 'vinavi_doctor_specialty']);
      
      // Only use storage if API fetch didn't populate the fields
      const doctorNameInput = document.getElementById('doctorName');
      if (result.vinavi_doctor_name && (!doctorNameInput.value || doctorNameInput.value === '')) {
        doctorNameInput.value = result.vinavi_doctor_name;
        document.getElementById('signatureName').textContent = 'Dr. ' + result.vinavi_doctor_name;
        console.log('[Medical Letter] Doctor name loaded from storage:', result.vinavi_doctor_name);
      }
      
      const doctorCodeInput = document.getElementById('doctorCode');
      if (result.vinavi_doctor_code && (!doctorCodeInput.value || doctorCodeInput.value === '')) {
        doctorCodeInput.value = result.vinavi_doctor_code;
        console.log('[Medical Letter] Doctor code loaded from storage:', result.vinavi_doctor_code);
      }

      const doctorSpecialtyInput = document.getElementById('doctorSpecialty');
      if (result.vinavi_doctor_specialty && (!doctorSpecialtyInput.value || doctorSpecialtyInput.value === '')) {
        doctorSpecialtyInput.value = result.vinavi_doctor_specialty;
        document.getElementById('signatureSpecialty').textContent = formatConsultantSpecialty(result.vinavi_doctor_specialty) || '[Specialty]';
        console.log('[Medical Letter] Doctor specialty loaded from storage:', result.vinavi_doctor_specialty);
      }
    } catch (error) {
  ...
  async function fetchDoctorFromVinavi() {
    try {
      ...
      if (data.data) {
        ...
        if (specialty) {
          ...
          document.getElementById('signatureSpecialty').textContent = formatConsultantSpecialty(specialty) || '[Specialty]';
  ...
  function setupEventListeners() {
    ...
    const doctorSpecialtyInput = document.getElementById('doctorSpecialty');
    if (doctorSpecialtyInput) doctorSpecialtyInput.addEventListener('input', (e) => {
