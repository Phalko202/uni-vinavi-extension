...
function createWidget() {
  ...
  // Extract button
  document.getElementById('widget-extract').addEventListener('click', () => {
    const statusDiv = document.getElementById('widget-status');
    const outputDiv = document.getElementById('widget-output');
    const copyBtn = document.getElementById('widget-copy');
    const extractBtn = document.getElementById('widget-extract');
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        document.getElementById('widget-filter').style.display = 'block';
  ...
  // Copy button
  document.getElementById('widget-copy').addEventListener('click', () => {
    const statusDiv = document.getElementById('widget-status');
    if (extractedText) {
      navigator.clipboard.writeText(extractedText).then(() => {
        ...
        statusDiv.style.borderColor = 'rgba(76, 175, 80, 0.4)';
        setTimeout(() => {
          const hideWnl = document.getElementById('widget-hide-wnl').checked;
  ...
  // Filter checkbox
  document.getElementById('widget-hide-wnl').addEventListener('change', () => {
    if (allTestResults.length > 0) {
      const hideWnl = document.getElementById('widget-hide-wnl').checked;
      const displayData = displayResults(allTestResults, hideWnl);
      extractedText = displayData.output;
      document.getElementById('widget-output').textContent = displayData.output;
      const statusDiv = document.getElementById('widget-status');
      statusDiv.textContent = hideWnl 
        ? `Showing ${displayData.filteredCount} tests (${displayData.totalCount - displayData.filteredCount} WNL hidden)`
        : `Extracted ${displayData.totalCount} tests`;
