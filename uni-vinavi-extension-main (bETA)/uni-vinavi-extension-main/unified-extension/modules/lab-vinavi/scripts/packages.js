...
const PATIENT_BUNDLES_KEY = 'labPatientBundlesHMH';

// ========== AUTO-SAVE ==========
let _autoSaveTimer = null;

function _isSetBuilderOpen() {
...
function _setAutosaveIndicator(text, stateClass) {
  if (!_isSetBuilderOpen()) return;
  const el = document.getElementById('setBuilderAutosaveIndicator');
  if (!el) return;
  el.textContent = text;
...
function scheduleAutoSave() {
  ...
  _setAutosaveIndicator('Autosave: pending…', 'is-saving');

  _autoSaveTimer = setTimeout(() => {
    try {
      saveBundles(window._labBundles || []);
      savePatientBundles(window._patientBundles || {});
      console.log('[AutoSave] Bundles saved');
      try {
        window.VinaviSaveFile?.requestSave('lab-autosave');
      } catch (_) {}
      _setAutosaveIndicator(`Autosaved at ${_formatTime(new Date())}`, 'is-saved');
    } catch (e) {
      console.warn('[AutoSave] Save failed', e);
      _setAutosaveIndicator('Autosave: failed', 'is-error');
    }
  }, 2000);
}
...
setInterval(() => {
  try {
    ...
    savePatientBundles(window._patientBundles || {});
    try {
      window.VinaviSaveFile?.requestSave('lab-periodic-autosave');
    } catch (_) {}
    _setAutosaveIndicator(`Autosaved at ${_formatTime(new Date())}`, 'is-saved');
  } catch (e) {
    console.warn('[AutoSave] Periodic save failed', e);
    _setAutosaveIndicator('Autosave: failed', 'is-error');
  }
}, 30000);
...
function saveBundles(list) {
  try {
    ...
    try { window.CloudSync?.onLocalChange?.(STORAGE_KEY, raw); } catch (_) {}
  } catch (_) {}
}

function loadPatientBundles() {
  try {
    const raw = localStorage.getItem(PATIENT_BUNDLES_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw);
...
function savePatientBundles(obj) {
  try {
    const raw = JSON.stringify(obj || {});
    localStorage.setItem(PATIENT_BUNDLES_KEY, raw);
    try { window.CloudSync?.onLocalChange?.(PATIENT_BUNDLES_KEY, raw); } catch (_) {}
  } catch (_) {}
}
...
function saveCurrentBundle() {
  ...
  // Clear catalog selections so next "Create New" starts fresh
  const catalogFrame = document.getElementById('labCatalogFrameBundleModal');
  if (catalogFrame && catalogFrame.contentWindow) {
    catalogFrame.contentWindow.postMessage({ type: 'clearSelection' }, '*');
  }
...
window.addEventListener('message', (event) => {
  ...
  // Only process messages from bundle modal catalog
  const bundleModalCatalog = document.getElementById('labCatalogFrameBundleModal');
  const isFromBundleModalCatalog = !!(bundleModalCatalog && event.source === bundleModalCatalog.contentWindow);
  ...
  if (event.data.type === 'openFullscreenCatalog') {
    const preselectedFromIframe = Array.isArray(event.data.preselectedTests)
      ? event.data.preselectedTests
      : [];

    if (!window.fullscreenLabCatalog) {
      console.error('[Packages] Fullscreen catalog not available');
      if (typeof showToast === 'function') {
    ...
    // Merge current bundle selection with iframe selection (defensive)
    const existingTests = Array.isArray(window._bundleCreationTests) ? window._bundleCreationTests : [];
    const allPreselected = [...existingTests];
    preselectedFromIframe.forEach(test => {
      const exists = allPreselected.some(t =>
        (t.code && test.code && t.code === test.code) ||
        (t.asnd && test.asnd && t.asnd === test.asnd) ||
        (t.name && test.name && t.name === test.name)
      );
      if (!exists) allPreselected.push(test);
    });

    console.log('[Packages] Opening fullscreen catalog with', allPreselected.length, 'preselected tests');
    ...
    window.fullscreenLabCatalog.open({
      ...
      onConfirm: (selectedTests) => {
        const finalSelected = Array.isArray(selectedTests) ? selectedTests : [];
        window._bundleCreationTests = finalSelected.map(t => ({
          code: t.code || '',
          asnd: t.asnd || '',
          name: t.name || '',
          vinaviServiceId: t.vinaviServiceId || t.serviceId || null
        ...
        // Sync back to the bundle modal catalog iframe (checkboxes + search state)
        try {
          bundleModalCatalog.contentWindow.postMessage({
            type: 'syncTests',
            tests: finalSelected
          }, '*');
        } catch (e) {
          console.warn('[Packages] Failed to sync selection back to catalog iframe:', e);
        }

        if (typeof showToast === 'function') {
          showToast(`${finalSelected.length} test(s) selected`, 'success');
        }
      }
    });
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  // Pull latest cloud state before rendering, if user is signed in.
  try { await window.CloudSync?.pull?.(); } catch (_) {}
  initBundleUI();
  initWizardEvents();
