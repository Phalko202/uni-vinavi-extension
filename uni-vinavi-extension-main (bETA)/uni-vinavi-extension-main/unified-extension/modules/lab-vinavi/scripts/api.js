...
(function attachVinaviApi(globalScope) {
  ...
  /**
   * Search medicines from Vinavi database
   * @param {string} query - Search query
   * @param {number} pageSize - Number of results per page (default: 20)
   * @returns {Promise<Object>} - Medicine search results
   */
  async function searchMedicines(query, pageSize = 20) {
    if (!query) {
      return { data: [], meta: { total: 0 } };
    }

    // Use the correct API endpoint with proper headers
    const url = `https://api.aasandha.mv/medicines?filter[is_active]=true&filter[name]=${encodeURIComponent(query)}&page[size]=${pageSize}&sort=-created_at`;
    ...
    try {
      ...
      console.error('[API] searchMedicines error:', error);
      throw error;
    }
  }
  ...
  async function getMedicineDetails(medicineId) {
    ...
    try {
      ...
      console.error('[API] getMedicineDetails error:', error);
      throw error;
    }
  }
