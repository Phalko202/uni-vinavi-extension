...
function setTheme(theme) {
  ...
  try {
    ...
    try {
      window.VinaviSaveFile?.requestSave('quicktext-theme');
    } catch (e) {}
  } catch (e) {}
}
...
async function loadTemplates() {
  // Load built-in templates from builtin-templates.js
  if (typeof VINAVI_BUILTIN_TEMPLATES !== 'undefined' && Object.keys(VINAVI_BUILTIN_TEMPLATES).length > 0) {
    builtinTemplates = JSON.parse(JSON.stringify(VINAVI_BUILTIN_TEMPLATES));
    console.log('✓ Built-in templates loaded:', Object.keys(builtinTemplates).length);
  } else {
    builtinTemplates = {};
    console.log('ℹ️ No built-in templates found');
  }
...
async function saveCustomTemplates() {
  try {
    ...
    try {
      window.VinaviSaveFile?.requestSave('quicktext-templates');
    } catch (e) {}
  } catch (e) {
...
function renderAllTemplates() {
  ...
  var allTemplates = {};
  Object.keys(builtinTemplates).forEach(function(key) {
    allTemplates[key] = builtinTemplates[key];
    allTemplates[key]._builtin = true;
  });
  Object.keys(customTemplates).forEach(function(key) {
    allTemplates[key] = customTemplates[key];
  });

  var entries = Object.keys(allTemplates).map(function(key) {
    return { key: key, template: allTemplates[key] };
  });
...
function editTemplate(key) {
  var template = customTemplates[key] || builtinTemplates[key];
  if (!template) return;
