...
(function() {
  ...
  function loadBuiltinTemplates() {
    // Load built-in templates from builtin-templates.js (injected before content.js)
    if (typeof VINAVI_BUILTIN_TEMPLATES !== 'undefined' && Object.keys(VINAVI_BUILTIN_TEMPLATES).length > 0) {
      for (const [key, template] of Object.entries(VINAVI_BUILTIN_TEMPLATES)) {
        TEMPLATES[key] = template;
      }
      console.log('✓ Built-in templates loaded:', Object.keys(VINAVI_BUILTIN_TEMPLATES).length);
      return true;
    }
    // Legacy: also check VINAVI_TEMPLATES
    if (typeof VINAVI_TEMPLATES !== 'undefined' && Object.keys(VINAVI_TEMPLATES).length > 0) {
      TEMPLATES = { ...VINAVI_TEMPLATES };
      console.log('✓ Built-in templates loaded (legacy):', Object.keys(TEMPLATES).length);
      return true;
    }
  ...
  function findShortcut(text, cursorPos) {
    ...
    const match = beforeCursor.match(/\/([a-zA-Z0-9_]*)$/);  // includes _ for /arthritis_assessment etc.
    ...
    try {
      ...
      if (template.mainLabels !== undefined && template.mainLabels !== null) {
        ...
        if (template.mainLabels.length === 0) {
          ...
          // Detect compact mode: all mainLabels have noHeader:true → use inline row design
          const isCompact = template.mainLabels.every(ml => !!ml.noHeader);
          if (isCompact) popup.classList.add('vinavi-compact');
          ...
          for (let idx = 0; idx < template.mainLabels.length; idx++) {
            ...
            // Main label header — only shown when noHeader is NOT set
            if (!mainLabel.noHeader) {
              fieldsHtml += `
            ...
            if (mainLabel.subLabels && Array.isArray(mainLabel.subLabels) && mainLabel.subLabels.length > 0) {
              ...
              for (let i = 0; i < mainLabel.subLabels.length; i++) {
                ...
                fieldsHtml += renderField(field, { fieldId, compact: isCompact });
      ...
      // Compact mode: minimal header, no gradient banner
      const isCompactPopup = popup.classList.contains('vinavi-compact');
      if (isCompactPopup) {
        popup.innerHTML = `
        <div class="vinavi-compact-header">
          <span class="vinavi-compact-title">${key}</span>
          <span class="vinavi-compact-subtitle">${template.title}</span>
          <button class="vinavi-popup-close vinavi-compact-close">×</button>
        </div>
        <div class="vinavi-popup-body vinavi-compact-body">
          <div class="vinavi-compact-rows">${fieldsHtml}</div>
        </div>
        <div class="vinavi-compact-footer">
          <button class="vinavi-compact-btn vinavi-compact-btn-cancel" id="vinavi-cancel">Cancel</button>
          <button class="vinavi-compact-btn vinavi-compact-btn-insert" id="vinavi-insert">Insert</button>
        </div>
      `;
        popup.style.cssText = `
          position: fixed !important;
  ...
  function renderField(field, ctx) {
    ...
    // ── COMPACT INLINE ROW MODE ─────────────────────────────────────────────
    // Renders as:  Label: [input/select]  all on one line (Text Blaze style)
    if (compact) {
      ...
      if (field.type === 'select') {
        ...
        return `
          <div class="vinavi-compact-row">
            <label class="vinavi-compact-label">${labelText}</label>
            <select class="vinavi-compact-select" data-id="${fieldId}">${opts}</select>
          </div>
        `;
      } else if (field.type === 'prefix' || field.type === 'text') {
        return `
          <div class="vinavi-compact-row">
            <label class="vinavi-compact-label">${labelText}</label>
            <input type="text" class="vinavi-compact-input" data-id="${fieldId}"
        ...
        return `
          <div class="vinavi-compact-row">
            <label class="vinavi-compact-label">${labelText}</label>
            <input type="text" inputmode="decimal" class="vinavi-compact-input" data-id="${fieldId}"
                   placeholder="${field.placeholder || ''}" value="${field.default || ''}">
            ${field.unit ? `<span class="vinavi-compact-unit">${field.unit}</span>` : ''}
          </div>
        `;
        ...
        return `
          <div class="vinavi-compact-row">
            <label class="vinavi-compact-label">${labelText}</label>
            <input type="checkbox" class="vinavi-checkbox vinavi-compact-checkbox" data-id="${fieldId}" ${field.default ? 'checked' : ''}>
          </div>
        `;
      }
      // Fallback for other types in compact mode
      return `
        <div class="vinavi-compact-row">
          <label class="vinavi-compact-label">${labelText}</label>
          <input type="text" class="vinavi-compact-input" data-id="${fieldId}" value="${field.default || ''}">
        </div>
      `;
    }
    // ── END COMPACT MODE ────────────────────────────────────────────────────
    ...
    if (field.type === 'select' || field.type === 'medication') {
      ...
      // Prefix type: inline sentence opener e.g. "Neck pain for [value]"
      // Renders as a regular text input; in output the value is appended to the label (no colon)
      html = `
        <div class="vinavi-field-group ${widthClass}">
          <label class="vinavi-field-label">${field.label}</label>
          <input type="text" class="vinavi-field-input" data-id="${fieldId}"
                 placeholder="${field.placeholder || 'e.g. 2 weeks'}" value="${field.default || ''}">
        </div>
      `;
  ...
  async function insertText() {
    ...
    if (template.format && typeof template.format === 'function') {
      ...
      template.mainLabels.forEach((mainLabel, mainIndex) => {
        ...
        // noHeader: skip the "Section:" header line; use no indentation for fields
        const noHeader = !!mainLabel.noHeader;
        const indent = noHeader ? '' : '  ';
        ...
        if (mainLabel.subLabels && Array.isArray(mainLabel.subLabels)) {
          mainLabel.subLabels.forEach((field, fieldIndex) => {
            ...
            // PREFIX type: sentence-starter e.g. "Neck pain for 1 week" (no colon)
            if (field.type === 'prefix') {
              const v = String(value || '').trim();
              if (v) {
                hasAnyContent = true;
                hasNonAdviceContent = true;
                sectionLines.push(`${indent}${field.label} ${v}`);
              }
              return;
            }
            ...
            if (field.type === 'advice') {
              ...
              if (selected.length > 0) {
                ...
                selected.forEach(item => {
                  ...
                  sectionLines.push(`${indent}• ${parts[0]}`);
                  for (let i = 1; i < parts.length; i++) {
                    sectionLines.push(`${indent}  ${parts[i]}`);
                  }
                });
              }
              return;
            ...
            if (Array.isArray(value)) {
              ...
              if (selected.length > 0) {
                hasAnyContent = true;
                hasNonAdviceContent = true;
                sectionLines.push(`${indent}${field.label}: ${selected.join(', ')}`);
              }
              return;
            }
            ...
            if (field.type === 'checkbox') {
              if (value === true || value === 'true') {
                hasAnyContent = true;
                hasNonAdviceContent = true;
                sectionLines.push(`${indent}${field.label}: Yes`);
              }
              return;
            }
            ...
            if (displayValue && String(displayValue).trim()) {
              hasAnyContent = true;
              hasNonAdviceContent = true;
              sectionLines.push(`${indent}${field.label}: ${String(displayValue).trim()}`);
            }
          });
        }
        ...
        // Print main label header only when noHeader is NOT set
        if (!noHeader && hasNonAdviceContent) {
          lines.push(mainLabel.name + ':');
        }

        lines.push(...sectionLines);
        if (!noHeader) lines.push(''); // Blank line between named sections
      });
  ...
  function handleKeyDown(e) {
    // ── DIRECT EXPANSION ──────────────────────────────────────────────────────
    // When the user presses Space/Enter/Tab right after a complete shortcut
    // (exact key match), skip the dropdown and open the popup immediately.
    if (e.key === ' ' || e.key === 'Enter' || e.key === 'Tab') {
      if (!currentPopup) {
        const el = findInputElement(e.target || document.activeElement);
        if (el) {
          const text = getText(el);
          const cursor = getCursorPos(el);
          const shortcutInfo = findShortcut(text, cursor);
          if (shortcutInfo) {
            const sc = shortcutInfo.shortcut; // already includes leading /
            if (TEMPLATES[sc]) {
              e.preventDefault();
              e.stopPropagation();
              hideSuggestions();
