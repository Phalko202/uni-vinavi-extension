...
/**
 * Format date as "20 Feb 2025 (Sunday)" — used in episode cards
 */
...
async function getDoctorSpecialty(professionalId) {
    ...
    // Check cache first — use `in` so null (no specialty / 403) is also cached
    if (professionalId in doctorSpecialtyCache) {
...
function showDoctorDetailsModal(professional, employee, userData, specialities = []) {
            ...
            <div class="doctor-details-modal">
                ...
                <div class="doctor-details-body">
                    <div class="doctor-profile-grid">
                        ...
                        <div class="detail-section full-width">
                            <h3><span class="section-icon">✍️</span> Medical Letter Specialty (Override)</h3>
                            <div class="doctor-override-help">Saved once and used to auto-fill Medical Letter. If you are signed into Cloud Account, it will sync to your other devices.</div>
                            <div class="doctor-override-form">
                                <input type="text" id="doctorSpecialtyOverrideInput" class="doctor-override-input" placeholder="Type your specialty (e.g., Consultant Cardiologist)">
                                <button id="doctorSpecialtyOverrideSave" class="doctor-override-btn">Save</button>
                            </div>
                            <div id="doctorSpecialtyOverrideStatus" class="doctor-override-status"></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // Wire specialty override editor
    try {
        const specialtyInput = document.getElementById('doctorSpecialtyOverrideInput');
        const specialtySaveBtn = document.getElementById('doctorSpecialtyOverrideSave');
        const specialtyStatus = document.getElementById('doctorSpecialtyOverrideStatus');

        const setStatusText = (text) => {
            if (specialtyStatus) specialtyStatus.textContent = text || '';
        };

        const saveSpecialtyOverride = () => {
            if (!specialtyInput) return;
            const value = (specialtyInput.value || '').trim();
            try {
                chrome.storage.local.set({ vinavi_doctor_specialty: value }, () => {
                    setStatusText(value ? 'Saved' : 'Cleared');
                    try { window.showToast?.('Doctor specialty saved', 'success'); } catch (_) {}
                    try { window.CloudSync?.onLocalChange?.('vinavi_doctor_specialty'); } catch (_) {}
                });
            } catch (e) {
                setStatusText('Save failed');
                try { window.showToast?.('Failed to save doctor specialty', 'error'); } catch (_) {}
                console.warn('[Doctor Profile] Failed saving specialty override:', e);
            }
        };

        if (specialtyInput) {
            try {
                chrome.storage.local.get(['vinavi_doctor_specialty'], (items) => {
                    const saved = (items?.vinavi_doctor_specialty || '').trim();
                    if (saved) {
                        specialtyInput.value = saved;
                        setStatusText('Loaded saved specialty');
                    } else if (specialtyDisplay && specialtyDisplay !== 'Not specified') {
                        specialtyInput.value = specialtyDisplay;
                        setStatusText('');
                    }
                });
            } catch (_) {}

            if (!specialtyInput._wired) {
                ...
                specialtyInput._wired = true;
            }
        }

        if (specialtySaveBtn && !specialtySaveBtn._wired) {
            specialtySaveBtn.addEventListener('click', saveSpecialtyOverride);
            specialtySaveBtn._wired = true;
        }
    } catch (e) {
        console.warn('[Doctor Profile] Specialty override wiring failed:', e);
    }
...
function initializeNavigation() {
    ...
    const activateView = (viewName) => {
        ...
        if (viewName === 'failLog') {
            try { window.LogView?.render?.(); } catch (_) { renderFailedTestsLog(); }
        }
        if (viewName === 'quickLinks') {
            try { window.QuickLinks?.init?.(); } catch (_) { }
        }
    };
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            // Restricted mode: allow Quick Links even without Vinavi portal login
            if (window._vinaviRestrictedMode && viewName !== 'quickLinks') {
                try {
                    window.showToast?.('Sign in to Vinavi to use this section. Quick Links is available without sign-in.', 'info');
                } catch (_) {}
                try { activateView('quickLinks'); } catch (_) {}
                return;
            }
...
function setRestrictedModeUI(isRestricted) {
    window._vinaviRestrictedMode = !!isRestricted;

    // Show prominent banner
    const banner = document.getElementById('aasandhaSignInBanner');
    if (banner) banner.classList.toggle('hidden', !isRestricted);

    // Disable/enable nav items visually (but keep click handlers for toast)
    document.querySelectorAll('.nav-item').forEach((item) => {
        const view = item.dataset?.view;
        const shouldDisable = !!isRestricted && view !== 'quickLinks';
        item.classList.toggle('is-disabled', shouldDisable);
        if (shouldDisable) item.setAttribute('aria-disabled', 'true');
    ...
    if (syncStatus) {
        const circle = syncStatus.querySelector('circle');
        const label = syncStatus.querySelector('span');
        if (circle) circle.setAttribute('fill', isRestricted ? '#ef4444' : '#10b981');
        if (label) label.textContent = isRestricted ? 'Not signed in to Aasandha (Quick Links available)' : 'Connected to Vinavi';
    }

    // Update footer user card text
    const userName = document.getElementById('userName');
    const userSpecialty = document.getElementById('userSpecialty');
    const userRole = document.getElementById('userRole');
    if (isRestricted) {
        if (userName) userName.textContent = 'Not signed in';
        if (userSpecialty) userSpecialty.textContent = '';
...
/**
 * Lazily fetch and cache the set of patient IDs that have episodes
 * assigned to the currently logged-in doctor.
 * Returns a Set<string> of patient IDs, or null if it can't be resolved.
 */
async function getOrFetchDoctorPatientIds() {
    // Return cached set if already populated
    if (window._vinaviDoctorPatientIds instanceof Set) {
        return window._vinaviDoctorPatientIds;
    }

    try {
        // Step 1: get the professional ID for the logged-in user
        if (!window._vinaviDoctorProfessionalId) {
            const authData = await checkAuthentication();
            const professionalItem = authData?.included?.find(i => i.type === 'professionals');
            if (!professionalItem?.id) {
                console.warn('[DoctorFilter] Could not resolve professional ID from auth data');
                return null;
            }
            window._vinaviDoctorProfessionalId = professionalItem.id;
        }

        const profId = window._vinaviDoctorProfessionalId;
        console.log('[DoctorFilter] Professional ID:', profId);

        // Step 2: fetch all outpatient episodes for this professional (page 1 — last 100)
        const epUrl = `https://vinavi.aasandha.mv/api/professionals/${profId}/episodes?filter[patient_type]=outpatient&include=patient&page[size]=100&sort=-created_at`;
        const response = await fetch(epUrl, { credentials: 'include' });
        if (!response.ok) throw new Error(`Episodes fetch failed: ${response.status}`);

        const epData = await response.json();
        const patientIds = new Set();
        (epData.included || []).forEach(item => {
            if (item.type === 'patients' && item.id) patientIds.add(String(item.id));
        });

        window._vinaviDoctorPatientIds = patientIds;
        console.log('[DoctorFilter] Loaded', patientIds.size, 'patients from doctor episodes');
        return patientIds;

    } catch (err) {
        console.warn('[DoctorFilter] Could not fetch doctor patient list:', err.message);
        return null; // Fail open — don't block the search
    }
}

/**
 * Show a styled warning modal when a searched patient has no episode
 * linked to the current doctor.  Resolves true = proceed, false = cancel.
 */
function confirmPatientNotInDoctorList(patientName) {
    return new Promise((resolve) => {
        document.getElementById('doctor-patient-warn-popup')?.remove();

        const popup = document.createElement('div');
        popup.id = 'doctor-patient-warn-popup';
        popup.style.cssText = `
            position: fixed; top: 0; left: 0; width: 100%; height: 100%;
            background: rgba(0,0,0,0.78); z-index: 100010;
            display: flex; align-items: center; justify-content: center;
        `;

        popup.innerHTML = `
            <div style="
                background: #1c1c1e; border-radius: 16px; padding: 28px 28px 22px;
                max-width: 420px; width: 90%;
                box-shadow: 0 24px 64px rgba(0,0,0,0.9), 0 0 0 1px rgba(255,80,80,0.25);
            ">
                <div style="font-size: 32px; margin-bottom: 12px;">⚠️</div>
                <h3 style="color: #ff6b6b; font-size: 16px; font-weight: 700; margin: 0 0 10px;">
                    Patient Not in Your Episode List
                </h3>
                <p style="color: rgba(255,255,255,0.65); font-size: 13px; margin: 0 0 22px; line-height: 1.5;">
                    <strong style="color: #fff;">${patientName || 'This patient'}</strong>
                    does not appear in your recent outpatient episodes.
                    Proceeding could result in pushing data to an episode that isn't assigned to you.
                </p>
                <div style="display: flex; gap: 10px;">
                    <button id="warn-proceed" style="
                        flex: 1; background: linear-gradient(135deg, #c0392b, #922b21);
                    ...
                    <button id="warn-cancel" style="
                        ...
                        padding: 12px; border-radius: 9px; cursor: pointer; font-size: 13px;
                    ">Cancel</button>
                </div>
            </div>
        `;
        ...
        popup.querySelector('#warn-proceed').addEventListener('click', () => { popup.remove(); resolve(true); });
        popup.querySelector('#warn-cancel').addEventListener('click', () => { popup.remove(); resolve(false); });
...
async function performPatientSearch() {
    ...
    try {
        ...
        // ── Doctor-filter safety check ─────────────────────────────────────
        // Warn the doctor if the patient has no episode in their list, so they
        // don't accidentally push data to another doctor's episode.
        const doctorPatientIds = await getOrFetchDoctorPatientIds();
        if (doctorPatientIds !== null && !doctorPatientIds.has(String(patientId))) {
            const patientName = searchData.data?.attributes?.name || searchData.data?.attributes?.['full-name'] || '';
            showLoading(false);
            const shouldProceed = await confirmPatientNotInDoctorList(patientName);
            if (!shouldProceed) return; // Abort search
            showLoading(true);
        }
        // ──────────────────────────────────────────────────────────────────
 ...
 * Fetches the last 30 episodes for the current patient, extracts all medications
 * across prescriptions, groups them by name, counts occurrences, and flags
 * anything given 3 or more times (or with long-duration instructions) as chronic.
 */
async function showChronicMedTracker(allCases, baseIncluded) {
    // Show a loading overlay immediately
    document.getElementById('chronic-tracker-overlay')?.remove();

    const loadingOverlay = document.createElement('div');
    loadingOverlay.id = 'chronic-tracker-overlay';
    loadingOverlay.style.cssText = `
        position:fixed;top:0;left:0;width:100%;height:100%;
        background:rgba(0,0,0,0.75);z-index:100020;
        display:flex;align-items:center;justify-content:center;
    ...
    loadingOverlay.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:40px;text-align:center;max-width:400px;">
            ...
            <h3 style="margin:0 0 8px;color:#1e293b;">Analyzing Medications</h3>
            <p id="chronic-loading-status" style="color:#64748b;font-size:14px;margin:0 0 20px;">Fetching episode data...</p>
            <div style="height:4px;background:#e2e8f0;border-radius:2px;overflow:hidden;">
                <div id="chronic-progress-bar" style="height:100%;background:linear-gradient(90deg,#7c3aed,#5b21b6);border-radius:2px;width:0%;transition:width 0.3s;"></div>
            </div>
        </div>
    `;
    ...
    const setStatus = (txt, pct) => {
        const s = document.getElementById('chronic-loading-status');
        const b = document.getElementById('chronic-progress-bar');
        if (s) s.textContent = txt;
        if (b) b.style.width = pct + '%';
    };

    try {
        // Limit to 30 most recent cases
        const casesToFetch = allCases.slice(0, 30);

        // Collect episode IDs from case last-episodes
        const episodeIds = casesToFetch
            .map(c => c.relationships?.['last-episode']?.data?.id)
            .filter(Boolean);

        if (episodeIds.length === 0) {
            loadingOverlay.remove();
            showToast('No episodes found for this patient', 'info');
            return;
        }

        setStatus(`Fetching ${episodeIds.length} episodes...`, 10);

        // Fetch each episode in parallel (batch of 6)
        const batchSize = 6;
        const allEpisodeData = [];
        for (let i = 0; i < episodeIds.length; i += batchSize) {
            const batch = episodeIds.slice(i, i + batchSize);
            const results = await Promise.all(batch.map(id =>
                fetch(`https://vinavi.aasandha.mv/api/episodes/${id}?include=prescriptions.medicines.preferred-medicine,diagnoses.icd-code`, { credentials: 'include' })
                    .then(r => r.ok ? r.json() : null)
                    .catch(() => null)
            ));
            allEpisodeData.push(...results.filter(Boolean));
            const pct = 10 + Math.round(((i + batchSize) / episodeIds.length) * 75);
            setStatus(`Processed ${Math.min(i + batchSize, episodeIds.length)} / ${episodeIds.length} episodes...`, Math.min(pct, 85));
        }

        setStatus('Analyzing medications...', 90);

        // --- Extract medications across all episodes ---
        // Map: normalized_name → { name, count, diagnoses: Set, episodes: [{date, episodeId, instructions}] }
        const medMap = new Map();

        for (const epData of allEpisodeData) {
            const ep = epData.data;
            const inc = epData.included || [];
            const epId = ep.id;
            const epDate = ep.attributes?.created_at || ep.attributes?.date;

            // Get diagnoses for this episode — prefer diagnosis description/name over raw icd code
            const icdCodeMap = {};
            inc.filter(i => i.type === 'icd-codes').forEach(icd => {
                icdCodeMap[icd.id] = {
                    code: icd.attributes?.code || '',
                    name: icd.attributes?.name || icd.attributes?.description || icd.attributes?.title || ''
                };
            });
            const diagnosesList = [];
            const diagnosisItems = inc.filter(i => i.type === 'diagnoses');
            if (diagnosisItems.length > 0) {
                diagnosisItems.forEach(diag => {
                    const icdRef = diag.relationships?.['icd-code']?.data;
                    const icd = icdRef ? (icdCodeMap[icdRef.id] || {}) : {};
                    const code = icd.code || diag.attributes?.code || '';
                    const name = icd.name ||
                        diag.attributes?.name ||
                        diag.attributes?.description ||
                        diag.attributes?.title ||
                        diag.attributes?.icd_code_description || '';
                    const display = code && name ? `${code} - ${name}` : (name || code);
                    if (display) diagnosesList.push(display);
                });
            } else {
                // Fallback: use icd-codes directly
                inc.filter(i => i.type === 'icd-codes').forEach(icd => {
                    const code = icd.attributes?.code || '';
                    const name = icd.attributes?.name || icd.attributes?.description || icd.attributes?.title || '';
            ...
            const prescRels = ep.relationships?.prescriptions?.data || [];
            for (const prescRel of prescRels) {
                const prescription = inc.find(i => i.type === 'prescriptions' && i.id === prescRel.id);
                if (!prescription) continue;

                const medRels = prescription.relationships?.medicines?.data || [];
                for (const medRel of medRels) {
                    const med = inc.find(i =>
                        (i.type === 'medicines' || i.type === 'prescription-medicines') && i.id === medRel.id
                    );
                    if (!med) continue;

                    const prefMedRel = med.relationships?.['preferred-medicine']?.data;
                    const prefMed = prefMedRel ? inc.find(i => i.type === 'preferred-medicines' && i.id === prefMedRel.id) : null;
                    const rawName = med.attributes?.name || prefMed?.attributes?.name || '';
                    const instructions = med.attributes?.instructions || '';
                    if (!rawName) continue;
                    ...
                    entry.count++;
                    diagnosesList.forEach(d => entry.diagnoses.add(d));
                    entry.episodes.push({ date: epDate, episodeId: epId, instructions });
                }
            }
        }
        ...
        if (medMap.size === 0) {
            showToast('No medications found across recent episodes', 'info');
            return;
        }

        // --- Duration category helper ---
        // Returns 'short' (<1 month), 'medium' (1-3 months), 'long' (>3 months), 'unknown'
        const getInstructionsDuration = (instructions) => {
            if (!instructions) return 'unknown';
            const text = instructions.toLowerCase();
            if (/ongoing|long[\s-]?term|chronic|permanent|indefinit|lifelong/i.test(text)) return 'long';
            ...
            if (/monthly/.test(text)) return 'medium';
            const weekMatch = text.match(/(\d+)\s*week/);
            if (weekMatch) { return parseInt(weekMatch[1]) <= 4 ? 'short' : 'medium'; }
            const dayMatch = text.match(/(\d+)\s*day/);
            if (dayMatch) { return parseInt(dayMatch[1]) <= 30 ? 'short' : 'medium'; }
            return 'unknown';
        };

        const getMedDuration = (med) => {
            // Use the most specific (longest) duration found across all episodes for this med
            const priority = { long: 3, medium: 2, short: 1, unknown: 0 };
            let best = 'unknown';
            for (const ep of med.episodes) {
                const d = getInstructionsDuration(ep.instructions);
                if (priority[d] > priority[best]) best = d;
            }
            return best;
        };

        // --- Flag chronic medications ---
        // Attach duration first so the filter can use it
        const allMeds = Array.from(medMap.values()).sort((a, b) => b.count - a.count);
        allMeds.forEach(m => { m._duration = getMedDuration(m); });

        // Chronic = only medications with instructions indicating > 3 months / ongoing / long-term
        const chronicMeds = allMeds.filter(m => m._duration === 'long');

        // --- Build popup ---
        const popup = document.createElement('div');
        popup.id = 'chronic-tracker-overlay';
        popup.style.cssText = `
            position:fixed;top:0;left:0;width:100%;height:100%;
            background:rgba(0,0,0,0.75);z-index:100020;
        ...
        const durLabels = { short:'< 1 Month', medium:'1–3 Months', long:'> 3 Months', unknown:'Unspecified' };
        const durColors = { short:'#d1fae5;color:#065f46', medium:'#fef3c7;color:#92400e', long:'#fee2e2;color:#991b1b', unknown:'#f1f5f9;color:#475569' };

        const renderMedRow = (m) => {
            const ischronic = chronicMeds.includes(m);
            const countColor = m.count >= 5 ? '#ef4444' : m.count >= 3 ? '#f59e0b' : '#10b981';
            const diagList = Array.from(m.diagnoses).slice(0, 4).join('; ') || '—';
            const durLabel = durLabels[m._duration] || 'Unspecified';
            const durBg = durColors[m._duration] || durColors.unknown;
            const rowId = 'medrow-' + m.name.replace(/[^a-z0-9]/gi,'_');
            const popupRef = popup; // captured for View button closure
            const epRowsHtml = m.episodes.slice().reverse().map(ep => {
                return `
                <tr style="border-top:1px solid #f1f5f9;">
                    <td style="padding:7px 10px;font-size:12px;color:#374151;white-space:nowrap;">${fmtEpDate(ep.date)}</td>
                    <td style="padding:7px 10px;font-size:12px;color:#7c3aed;font-weight:600;">EP-${ep.episodeId}</td>
                    <td style="padding:7px 10px;font-size:12px;color:#64748b;">${escapeHtml(ep.instructions || '—')}</td>
                    <td style="padding:7px 10px;white-space:nowrap;">
                        <button class="cmed-view-ep" data-epid="${ep.episodeId}" style="
                            padding:3px 10px;font-size:11px;font-weight:600;border:none;
                            background:#ede9fe;color:#7c3aed;border-radius:8px;cursor:pointer;
                        ">View ↗</button>
                    </td>
                </tr>`;
            }).join('');
            ...
            return `
            <div class="chronic-med-row" data-duration="${m._duration}" data-ischronic="${ischronic}"
                 style="border:1px solid ${ischronic ? '#fde68a' : '#e2e8f0'};border-radius:12px;
                        margin-bottom:8px;background:${ischronic ? '#fffbeb' : '#fafafa'};
                        overflow:hidden;transition:box-shadow .15s;">
                <!-- Header row — click to expand -->
                <div class="chronic-med-header" style="
                ...
                ">
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;min-width:0;">
                        <span style="font-weight:700;font-size:14px;color:#1e293b;">${escapeHtml(m.name)}</span>
                        <span style="background:${countColor};color:#fff;font-size:11px;font-weight:700;
                              padding:2px 9px;border-radius:20px;flex-shrink:0;">×${m.count} episodes</span>
                        <span style="background:${durBg};font-size:11px;font-weight:600;
                              padding:2px 9px;border-radius:20px;flex-shrink:0;">${durLabel}</span>
                        ${ischronic ? `<span style="background:#fef3c7;color:#92400e;border:1px solid #fde68a;
                              font-size:11px;font-weight:600;padding:2px 9px;border-radius:20px;">⚠ Chronic</span>` : ''}
                    </div>
                    <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
                        <span style="font-size:11px;color:#94a3b8;">click to see episodes</span>
                        <span class="cmed-arrow" style="display:inline-block;transition:transform .2s;color:#94a3b8;font-size:14px;">▼</span>
                    </div>
                </div>
                <!-- Expandable body -->
                <div class="chronic-med-body" style="display:none;border-top:1px solid ${ischronic ? '#fde68a' : '#e2e8f0'};">
                    ${diagList !== '—' ? `<div style="padding:8px 16px;font-size:12px;color:#64748b;background:${ischronic?'#fefce8':'#f8fafc'};border-bottom:1px solid ${ischronic?'#fde68a':'#e2e8f0'};">
                        <span style="font-weight:700;color:#374151;">Diagnoses: </span>${escapeHtml(diagList)}
                    </div>` : ''}
                    <div style="overflow-x:auto;">
                        <table style="width:100%;border-collapse:collapse;font-size:13px;">
                            <thead>
                                <tr style="background:${ischronic?'#fef9c3':'#f1f5f9'};">
                                    <th style="padding:7px 10px;text-align:left;font-size:11px;font-weight:700;color:#64748b;white-space:nowrap;">Date</th>
                                    <th style="padding:7px 10px;text-align:left;font-size:11px;font-weight:700;color:#64748b;white-space:nowrap;">Episode</th>
                                    <th style="padding:7px 10px;text-align:left;font-size:11px;font-weight:700;color:#64748b;">Instructions</th>
                                    <th style="padding:7px 10px;text-align:left;font-size:11px;font-weight:700;color:#64748b;"></th>
                                </tr>
                            </thead>
        ...
        const renderList = (filter, chronicOnly) => {
            const listEl = popup.querySelector('#chronic-med-list');
            if (!listEl) return;
            let meds = allMeds;
            if (chronicOnly) meds = meds.filter(m => chronicMeds.includes(m));
            if (filter !== 'all') meds = meds.filter(m => m._duration === filter);
            if (meds.length === 0) {
                listEl.innerHTML = '<p style="color:#94a3b8;font-size:13px;text-align:center;padding:20px 0;">No medications match the selected filter.</p>';
            } else {
                listEl.innerHTML = meds.map(m => renderMedRow(m)).join('');
            }
            // Wire accordion toggles (CSP blocks inline onclick — use event listeners)
            listEl.querySelectorAll('.chronic-med-header').forEach(header => {
                header.addEventListener('click', () => {
                    const body = header.nextElementSibling;
                    const arrow = header.querySelector('.cmed-arrow');
                    if (!body) return;
            ...
            // Wire View Episode buttons — open the Vinavi episode URL in a new tab
            listEl.querySelectorAll('.cmed-view-ep').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation(); // don't trigger accordion
                    const epId = btn.dataset.epid;
                    if (!epId) return;
                    const patientId = window.currentPatient?.id ||
                        window.currentPatient?.attributes?.patient_id ||
                        window.currentPatient?.attributes?.id;
                    const vinaviUrl = patientId
                        ? `https://vinavi.aasandha.mv/#/patients/${patientId}/episodes/${epId}`
                        : `https://vinavi.aasandha.mv/#/episodes/${epId}`;
        ...
        popup.innerHTML = `
            <div style="
                background:#fff;border-radius:18px;width:960px;max-width:96vw;
                max-height:88vh;display:flex;flex-direction:column;
                box-shadow:0 28px 80px rgba(0,0,0,0.38);overflow:hidden;
                margin:auto;
            ">
                <!-- Header -->
                <div style="
                ...
                ">
                    <div>
                        <h2 style="margin:0;color:#fff;font-size:20px;font-weight:700;">💊 Medication History & Chronic Tracker</h2>
                        <p style="margin:4px 0 0;color:rgba(255,255,255,0.75);font-size:13px;">
                            Analysed <strong style="color:#fff;">${allEpisodeData.length} episodes</strong> · 
                            <strong style="color:#fff;">${allMeds.length}</strong> unique medications · 
                            <strong style="color:#fde68a;">${chronicMeds.length} chronic</strong>
                        </p>
                    </div>
                    <button id="chronic-close" style="
                ...
                <!-- Filter bar -->
                <div style="
                    padding:14px 28px;border-bottom:1px solid #e2e8f0;background:#f8fafc;
                    flex-shrink:0;display:flex;align-items:center;gap:8px;flex-wrap:wrap;
                ">
                    <span style="font-size:12px;font-weight:700;color:#64748b;margin-right:4px;">DURATION:</span>
                    <button class="cmed-chip active" data-filter="all" style="
                        padding:5px 14px;border-radius:20px;border:1.5px solid #7c3aed;
                        background:#7c3aed;color:#fff;font-size:12px;font-weight:600;cursor:pointer;
                    ">All</button>
                    <button class="cmed-chip" data-filter="short" style="
                    ...
                    ">< 1 Month</button>
                    <button class="cmed-chip" data-filter="medium" style="
                        padding:5px 14px;border-radius:20px;border:1.5px solid #d1d5db;
                        background:#fff;color:#374151;font-size:12px;font-weight:600;cursor:pointer;
                    ">1–3 Months</button>
                    <button class="cmed-chip" data-filter="long" style="
                        padding:5px 14px;border-radius:20px;border:1.5px solid #d1d5db;
                        background:#fff;color:#374151;font-size:12px;font-weight:600;cursor:pointer;
                    ">> 3 Months</button>
                    <div style="width:1px;height:22px;background:#d1d5db;margin:0 4px;"></div>
                    <button class="cmed-chip cmed-chronic-toggle" data-filter-chronic="false" style="
                        padding:5px 14px;border-radius:20px;border:1.5px solid #fde68a;
                        background:#fff;color:#92400e;font-size:12px;font-weight:600;cursor:pointer;
                    ">⚠ Chronic Only (${chronicMeds.length})</button>
                </div>

                <!-- Med list -->
                <div style="flex:1;overflow-y:auto;padding:20px 28px;scrollbar-width:thin;" id="chronic-med-list">
                </div>

                <!-- Footer -->
                <div style="
                    padding:12px 24px;border-top:1px solid #e2e8f0;background:#f8fafc;
                    flex-shrink:0;font-size:11px;color:#94a3b8;text-align:center;
                ">
                    Based on last ${allEpisodeData.length} episodes · Chronic = given ≥3 times or instructions mention months/ongoing/long-term · Click any row to see episode history
                </div>
            </div>
        `;
        ...
        let chronicOnly = false;
        const chips = popup.querySelectorAll('.cmed-chip');
        const chronicToggle = popup.querySelector('.cmed-chronic-toggle');

        chips.forEach(chip => {
            chip.addEventListener('click', () => {
                if (chip === chronicToggle) {
                    // Toggle chronic only
                    ...
                    activeDuration = chip.dataset.filter;
                    chips.forEach(c => {
                        if (c === chronicToggle) return; // don't reset toggle
                ...
                renderList(activeDuration, chronicOnly);
            });
        });

        popup.querySelector('#chronic-close').addEventListener('click', () => popup.remove());
        // Popup only closes via the X button — not by clicking the dark overlay background

    } catch (err) {
        loadingOverlay.remove();
        console.error('[ChronicTracker] Error:', err);
        showToast('Failed to load medication history: ' + err.message, 'error');
    }
}
...
function displayPatientDetails(patient, cases, included = []) {
    ...
    // ── Doctor-only default: pre-filter to signed-in doctor's episodes ───
    const currentProfId = window._currentProfessional?.id;
    let defaultCases = cases;
    if (currentProfId) {
        const myCases = cases.filter(c => c.relationships?.doctor?.data?.id === currentProfId);
        if (myCases.length > 0) defaultCases = myCases;
    }
    if (epCount) epCount.textContent = String(defaultCases.length);
    // ─────────────────────────────────────────────────────────────────────

    // Show results filters + activate "My Episodes" chip by default
    const resultsFilters = document.getElementById('resultsFilters');
    if (resultsFilters) {
        resultsFilters.style.display = 'flex';
        // Pre-activate "My Episodes" if doctor is identified
        if (currentProfId) {
            resultsFilters.querySelectorAll('.filter-chip').forEach(c => c.classList.remove('active'));
            const myChip = resultsFilters.querySelector('[data-filter="my"]');
            if (myChip) myChip.classList.add('active');
        }
        initializeResultsFilters();
    }

    // Inject Medication History button next to section title (idempotent)
    let chronicBtn = document.getElementById('btn-chronic-tracker');
    if (!chronicBtn) {
        const sectionTitle = document.querySelector('#patientView .section-title');
        if (sectionTitle) {
            chronicBtn = document.createElement('button');
            chronicBtn.id = 'btn-chronic-tracker';
            chronicBtn.innerHTML = `<span>💊</span> Medication History`;
            Object.assign(chronicBtn.style, {
                marginLeft: '12px', padding: '7px 16px',
                background: 'linear-gradient(135deg, #7c3aed, #5b21b6)',
            ...
            sectionTitle.appendChild(chronicBtn);
        }
    }
    if (chronicBtn) {
        // Re-bind (patient may have changed)
        chronicBtn.onclick = () => showChronicMedTracker(cases, included);
    }

    // Patient Insights dropdown removed

    // Render episodes (doctor-filtered by default)
    displayEpisodes(defaultCases, included);
...
function initializeResultsFilters() {
    ...
    const customDateRange = document.getElementById('customDateFilter');
    const applyDateRangeBtn = document.getElementById('applyDateFilter');
    
    // Filter chips
    const filterChips = document.querySelectorAll('#resultsFilters .filter-chip');
    ...
    const applyFilters = () => {
        ...
        const activeChip = document.querySelector('#resultsFilters .filter-chip.active');
    ...
    if (filterChips.length > 0 && !filterChips[0]._filtersWired) {
        filterChips.forEach(chip => {
            chip.addEventListener('click', () => {
                ...
                // Show/hide custom date pane
                if (chip.dataset.filter === 'custom') {
                    if (customDateRange) customDateRange.style.display = 'flex';
                } else {
                    if (customDateRange) customDateRange.style.display = 'none';
                    applyFilters();
...
function displayEpisodes(cases, included = []) {
    ...
    // Today's episode from this doctor takes priority, otherwise the newest.
    const todayStr = new Date().toDateString();
    let highlightCaseId = null;
    const todayCase = cases.find(c => new Date(c.attributes?.created_at || '').toDateString() === todayStr);
    if (todayCase) {
        highlightCaseId = String(todayCase.id);
    } else if (cases.length > 0) {
        highlightCaseId = String(cases[0].id); // already sorted -created_at
    }
    // ─────────────────────────────────────────────────────────────────────
    ...
    listContainer.innerHTML = cases.map(caseItem => {
        ...
        const isHighlighted = String(caseItem.id) === highlightCaseId;
        const isTodayEp = new Date(attrs.created_at || '').toDateString() === todayStr;
        
        // Format date as "20 Feb 2025 (Sunday)"
        const dateFormatted = attrs.created_at ? formatEpisodeDate(attrs.created_at) : '-';
        ...
        // Highlighted card styles (today = bright green, most-recent = softer green)
        const highlightBorder = isTodayEp
            ? '2px solid #22c55e'
            : (isHighlighted ? '2px solid #4ade80' : '1px solid #e5e7eb');
        const highlightShadow = isHighlighted
            ? '0 0 0 4px rgba(34,197,94,0.12), 0 4px 16px rgba(34,197,94,0.1)'
            : '';
        const highlightBg = isTodayEp
            ? 'linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%)'
            : (isHighlighted ? 'linear-gradient(135deg, #f7fef9 0%, #ecfdf5 100%)' : '');
        
        const todayBadge = isTodayEp
            ? `<span style="
                background: #22c55e; color: #fff; font-size: 10px;
                font-weight: 700; padding: 2px 8px; border-radius: 20px;
                letter-spacing: 0.5px; text-transform: uppercase;">Today</span>`
            : (isHighlighted
                ? `<span style="
                    background: #86efac; color: #14532d; font-size: 10px;
                    font-weight: 700; padding: 2px 8px; border-radius: 20px;">Most Recent</span>`
                : '');
        ...
        return `
            <div class="episode-card" 
                 ...
                 style="border: ${highlightBorder}; ${highlightShadow ? `box-shadow: ${highlightShadow};` : ''} ${highlightBg ? `background: ${highlightBg};` : ''}">
                <div class="episode-header">
                    <div class="doctor-info-section">
                        <div class="doctor-name-large" style="${isTodayEp ? 'color: #15803d;' : ''}">${escapeHtml(doctorName)}</div>
...
window.selectEpisodeForLab = async function(episodeId) {
    ...
    try {
        ...
        try {
            ...
            // Extract medications with dose/instructions via prescription → medicine relationships
            const _prescItems = (window.currentEpisodeIncluded || []).filter(i => i.type === 'prescriptions');
            const _medObjects = [];
            if (_prescItems.length > 0) {
                _prescItems.forEach(presc => {
                    const medRels = presc.relationships?.medicines?.data || [];
                    medRels.forEach(medRel => {
                        const med = (window.currentEpisodeIncluded || []).find(i =>
                            (i.type === 'medicines' || i.type === 'prescription-medicines') && i.id === medRel.id
                        );
                        if (!med) return;
                        const attrs = med.attributes || {};
                        const preferred = attrs.preferred_medicine || {};
                        const name = attrs.name || preferred.name || '';
                        if (!name) return;
                        const strength = preferred.strength || attrs.strength || '';
                        const preparation = preferred.preparation || attrs.preparation || '';
                        const instructions = attrs.instructions || '';
                        const dose = [strength, preparation].filter(Boolean).join(' · ');
                        _medObjects.push({ name, dose, instruction: instructions });
                    });
                });
            }
            // Fallback to plain medicine items if no prescription structure found
            const meds = _medObjects.length > 0 ? _medObjects
                : (window.currentEpisodeIncluded || []).filter(i => i.type === 'medicines' || i.type === 'preferred-medicines')
                    .map(i => ({ name: i.attributes?.name || '', dose: '', instruction: '' })).filter(m => m.name);
            ...
            if (summaryEl) {
                ...
                const medsHtml = meds.length
                    ? meds.map(m => `
                        <div class="medication-card">
                          <div class="med-card-name">${escapeHtml(m.name)}</div>
                          ${m.dose ? `<div class="med-card-row"><span class="med-card-label">Dose</span><span class="med-card-value">${escapeHtml(m.dose)}</span></div>` : ''}
                          ${m.instruction ? `<div class="med-card-row"><span class="med-card-label">Instructions</span><span class="med-card-value">${escapeHtml(m.instruction)}</span></div>` : ''}
                        </div>`).join('')
                    : '<span class="detail-none">No medications prescribed</span>';
                ...
                // For read-only (another doctor's) episodes: auto-expand and hide the toggle button
                const _roDocId = episodeData.data?.relationships?.doctor?.data?.id;
                const _roMyId = window._currentProfessional?.id;
                if (_roDocId && _roMyId && String(_roDocId) !== String(_roMyId)) {
                    const toggleWrap = summaryEl.querySelector('.episode-drawer-toggle');
        ...
        // --- Ownership check: hide lab ordering UI for other doctors' episodes ---
        try {
            const epDoctorId = episodeData.data?.relationships?.doctor?.data?.id;
            const myId = window._currentProfessional?.id;
            const isMyEpisode = epDoctorId && myId && String(epDoctorId) === String(myId);

            const catalogSection = document.querySelector('.lab-catalog-section');
            ...
            const quickBundle = document.getElementById('quickBundleBtn');

            // Remove any existing read-only banner
            document.getElementById('lab-readonly-banner')?.remove();

            // Always clear the top link slot first
            const _topLinkSlot = document.getElementById('orderVinaviLinkSlot');
            if (_topLinkSlot) _topLinkSlot.innerHTML = '';

            if (!isMyEpisode && epDoctorId) {
                // Hide the ordering UI
                if (catalogSection) catalogSection.style.display = 'none';
                if (sidebar) sidebar.style.display = 'none';

                // Show a read-only notice
                const banner = document.createElement('div');
                banner.id = 'lab-readonly-banner';
                banner.style.cssText = `
                    ...
                    font-family:'Segoe UI',system-ui,sans-serif;
                `;
                const _roPatientId = window.currentPatient?.id || window.currentPatient?.attributes?.patient_id;
                const _roEpisodeId = episodeData.data?.id;
                const _roVinaviUrl = _roPatientId && _roEpisodeId
                    ? `https://vinavi.aasandha.mv/#/patients/${_roPatientId}/episodes/${_roEpisodeId}`
                    : null;

                // Inject View in Vinavi button into the top header slot
                if (_roVinaviUrl && _topLinkSlot) {
                    _topLinkSlot.innerHTML = `
                        <a href="${_roVinaviUrl}" target="_blank" style="
                            display:inline-flex;align-items:center;gap:6px;
                            padding:6px 14px;
                            background:linear-gradient(135deg,#1d4ed8,#2563eb);
                            color:#fff;border-radius:18px;font-size:12px;font-weight:700;
                ...
                banner.innerHTML = `
                    ...
                    <div style="flex:1;">
                        <div style="font-weight:700;font-size:14px;color:#92400e;">Read-Only View — Another Doctor's Episode</div>
                        <div style="font-size:13px;color:#a16207;margin-top:2px;">
                            Lab ordering, cart and test selection are only available for your own episodes.
                            You can view the episode summary above.
                        </div>
                    </div>
                    ${_roVinaviUrl ? `
                    <a id="btn-view-in-vinavi-ro" href="${_roVinaviUrl}" target="_blank" style="
                        display:inline-flex;align-items:center;gap:6px;
                        padding:8px 16px;
                        background:linear-gradient(135deg,#1d4ed8,#2563eb);
                    ...
                    " onmouseover="this.style.boxShadow='0 4px 14px rgba(37,99,235,0.45)'"
                      onmouseout="this.style.boxShadow='0 2px 8px rgba(37,99,235,0.3)'">
                        View in Vinavi ↗
                    </a>` : ''}
                `;
                const labContent = document.querySelector('.lab-content-wrapper') || document.getElementById('labOrderView');
                if (labContent) labContent.insertAdjacentElement('beforebegin', banner);
                else document.getElementById('episodeSummary')?.insertAdjacentElement('afterend', banner);
            } else {
                // My episode — restore to default
        ...
        // Wire Renew Rx header button — only for the signed-in doctor's own episodes
        const renewRxBtn = document.getElementById('renewRxHeaderBtn');
        if (renewRxBtn && episodeData.data) {
            const epDoctorId = episodeData.data.relationships?.doctor?.data?.id;
            const myId = window._currentProfessional?.id;
            const isMyEpisodeForRx = epDoctorId && myId && String(epDoctorId) === String(myId);

            if (isMyEpisodeForRx) {
                const doctorInc = (window.currentEpisodeIncluded || []).find(i =>
                    i.type === 'professionals' && i.id == epDoctorId
                );
                ...
                // Another doctor's episode — hide Renew Rx entirely
                renewRxBtn.style.display = 'none';
            }
        }

        // Listen for catalog messages (guard against duplicates)
        if (!window._catalogMessageListenerWired) {
            window.addEventListener('message', handleCatalogMessage);
            window._catalogMessageListenerWired = true;
        }
        showLoading(false);
...
function handleCatalogMessage(event) {
    // If message is coming from the set-builder catalog iframe, avoid
    // overwriting the main dashboard selection (set builder has its own listener).
    const setBuilderFrame = document.getElementById('setLabCatalogFrame');
    const isFromSetBuilderCatalog = !!(setBuilderFrame && event.source === setBuilderFrame.contentWindow);

    // Bundle modal catalog uses its own message handling (packages.js)
    const bundleModalFrame = document.getElementById('labCatalogFrameBundleModal');
    const isFromBundleModalCatalog = !!(bundleModalFrame && event.source === bundleModalFrame.contentWindow);

    if (event.data.type === 'testSelectionChanged') {
        if (isFromSetBuilderCatalog) return;
        ...
        if (isFromBundleModalCatalog) return;
        // If this request came from the set builder iframe, keep it in set-builder mode.
        if (isFromSetBuilderCatalog) {
            openFullscreenCatalogFromSetBuilder(event.data.preselectedTests || []);
        } else {
            // Open fullscreen catalog popup from main dashboard and sync back to the sender
            openFullscreenCatalogFromDashboard(event.data.preselectedTests || [], event.source);
        }
    }
}

/**
 * Open fullscreen catalog from set builder
 */
function openFullscreenCatalogFromSetBuilder(preselectedTests = []) {
    console.log('[SetBuilder] openFullscreenCatalogFromSetBuilder called with', preselectedTests.length, 'preselected tests');
    console.log('[SetBuilder] window.fullscreenLabCatalog exists:', !!window.fullscreenLabCatalog);
    
    if (window.fullscreenLabCatalog) {
        // Merge with current set's labs
        const existingTests = currentSetData.labs || [];
        ...
        console.log('[SetBuilder] Opening fullscreen with', allPreselected.length, 'preselected tests');
        ...
        window.fullscreenLabCatalog.open({
            ...
            onConfirm: (selectedTests) => {
                console.log('[SetBuilder] Tests selected from fullscreen:', selectedTests);
                // Update the set builder data
                currentSetData.labs = selectedTests;
                updateSetBuilderDisplay();
                
                // Sync with the set builder catalog iframe
                const frame = document.getElementById('setLabCatalogFrame');
        ...
        console.error('[SetBuilder] Fullscreen catalog not available');
 ...
 */
function openFullscreenCatalogFromDashboard(preselectedTests = [], sourceWindow = null) {
    if (window.fullscreenLabCatalog) {
        ...
        window.fullscreenLabCatalog.open({
            ...
            onConfirm: (selectedTests) => {
                ...
                // Sync back to whichever catalog iframe requested fullscreen.
                // Fallback to the main dashboard iframe if sourceWindow isn't provided.
                const targetWindow = sourceWindow || document.getElementById('labCatalogFrame')?.contentWindow;
                if (targetWindow) {
...
async function showEpisodeSelectorModal(episodes, targetEpisode) {
    ...
    // Meds shortcut button — keep modal open, show prescription choice popup
    overlay.querySelectorAll('.btn-meds-shortcut').forEach(btn => {
        btn.addEventListener('click', () => {
            const idx = parseInt(btn.getAttribute('data-episode-idx'), 10);
            const selectedEpisode = episodes[idx];
            // Do NOT close the modal — show choice popup on top
            showPrescriptionChoicePopup(selectedEpisode, targetEpisode);
        });
    });
}

/**
 * Show a choice popup asking the doctor whether to push meds to an
 * existing prescription or create a brand-new one.
 */
function showPrescriptionChoicePopup(selectedEpisode, targetEpisode) {
    // Remove any stale popup
    document.getElementById('prescription-choice-popup')?.remove();

    const popup = document.createElement('div');
    popup.id = 'prescription-choice-popup';
    popup.style.cssText = `
        position: fixed; top: 0; left: 0; width: 100%; height: 100%;
        background: rgba(0,0,0,0.75); z-index: 100002;
        display: flex; align-items: center; justify-content: center;
        animation: fadeInChoicePopup 0.18s ease-out;
    `;

    popup.innerHTML = `
        <style>
            @keyframes fadeInChoicePopup {
                from { opacity: 0; transform: scale(0.95); }
                to   { opacity: 1; transform: scale(1); }
            }
            #prescription-choice-popup .choice-card {
                background: #12221a;
                border-radius: 16px;
            ...
            #prescription-choice-popup h3 {
                color: #ffffff;
                font-size: 17px;
                font-weight: 700;
                margin: 0 0 6px;
            }
            #prescription-choice-popup p {
                color: rgba(255,255,255,0.55);
                font-size: 13px;
                margin: 0 0 22px;
            }
            #prescription-choice-popup .choice-btn {
                width: 100%;
                border: none;
                padding: 14px 18px;
                border-radius: 10px;
            ...
            #prescription-choice-popup .choice-btn:hover { filter: brightness(1.12); }
            #prescription-choice-popup .choice-btn .choice-icon { font-size: 22px; flex-shrink: 0; }
            #prescription-choice-popup .choice-btn .choice-label { font-size: 14px; font-weight: 600; }
            #prescription-choice-popup .choice-btn .choice-sub { font-size: 11px; font-weight: 400; opacity: 0.75; margin-top: 2px; }
            #prescription-choice-popup #choice-add-existing { background: linear-gradient(135deg, #1565c0, #0d47a1); }
            #prescription-choice-popup #choice-create-new   { background: linear-gradient(135deg, #2e7d32, #1b5e20); }
            #prescription-choice-popup #choice-cancel {
                width: 100%;
            ...
            #prescription-choice-popup #choice-cancel:hover { background: rgba(255,255,255,0.12); }
        </style>
        <div class="choice-card">
            <h3>💊 Prescription Action</h3>
            <p>Choose how medications should be added to this episode:</p>
            <button class="choice-btn" id="choice-add-existing">
                <span class="choice-icon">📋</span>
                <div>
                    <div class="choice-label">Push to Current Prescription</div>
                    <div class="choice-sub">Add medicines to the existing prescription in this episode</div>
                </div>
            </button>
            <button class="choice-btn" id="choice-create-new">
                <span class="choice-icon">📄</span>
                <div>
                    <div class="choice-label">Create New Prescription</div>
                    <div class="choice-sub">Start a fresh prescription and add medicines to it</div>
                </div>
            </button>
    ...
    popup.querySelector('#choice-add-existing').addEventListener('click', async () => {
        close();
        await renewPrescriptionsFromEpisode(selectedEpisode, targetEpisode, { medsOnly: true, useExistingPrescription: true });
    });

    popup.querySelector('#choice-create-new').addEventListener('click', async () => {
        close();
        await renewPrescriptionsFromEpisode(selectedEpisode, targetEpisode, { medsOnly: true });
    });

    popup.querySelector('#choice-cancel').addEventListener('click', close);
 ...
 */
/**
 * Shown when the user chooses "Push to Current Prescription" but no
 * prescription exists in the target episode.
 */
function showRxNoPrescriptionDialog() {
    _showRxInfoDialog(
        '📋 No Existing Prescription',
        '#1565c0', '#0d47a1',
        `<p style="color:#374151;font-size:14px;line-height:1.6;margin:0 0 10px;">
            This episode does not have an existing prescription yet.
        </p>
        <p style="color:#374151;font-size:14px;line-height:1.6;margin:0 0 18px;">
            Please go to Vinavi and <strong>create a prescription</strong> in this episode first,
            then come back and use <em>"Push to Current Prescription"</em>.
        </p>
        <p style="color:#64748b;font-size:13px;background:#f1f5f9;padding:10px 14px;border-radius:8px;margin:0;">
            Alternatively, use <strong>"Create New Prescription"</strong> — but a diagnosis must be
            added to the episode in Vinavi before the system will allow it.
        </p>`
    );
}

/**
 * Shown when Vinavi rejects prescription creation with 406
 * (diagnosis required before prescription).
 */
function showRxNoDiagnosisDialog() {
    _showRxInfoDialog(
        '⚠️ Diagnosis Required',
        '#b45309', '#92400e',
        `<p style="color:#374151;font-size:14px;line-height:1.6;margin:0 0 10px;">
            Vinavi requires <strong>a diagnosis to be added</strong> to the episode
            before a prescription can be created.
        </p>
        <p style="color:#374151;font-size:14px;line-height:1.6;margin:0 0 18px;">
            Please open this episode in Vinavi, add a diagnosis (ICD code), then
            return here and try again.
        </p>
        <p style="color:#64748b;font-size:13px;background:#fef3c7;border:1px solid #fde68a;
            padding:10px 14px;border-radius:8px;margin:0;">
            <strong>Tip:</strong> If you only want to add medications to an
            <em>existing</em> prescription, use the <strong>"Push to Current Prescription"</strong> option instead.
        </p>`
    );
}

function _showRxInfoDialog(title, gradFrom, gradTo, bodyHtml) {
    document.getElementById('rx-info-dialog-overlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'rx-info-dialog-overlay';
    overlay.style.cssText = `
        position:fixed;top:0;left:0;width:100%;height:100%;
        background:rgba(0,0,0,0.72);z-index:100030;
        display:flex;align-items:center;justify-content:center;
        font-family:'Segoe UI',system-ui,sans-serif;
    `;
    overlay.innerHTML = `
        ...
        ">
            <div style="background:linear-gradient(135deg,${gradFrom},${gradTo});padding:18px 22px;display:flex;align-items:center;justify-content:space-between;">
                <h3 style="margin:0;color:#fff;font-size:16px;font-weight:700;">${title}</h3>
                <button id="rx-info-close" style="background:rgba(255,255,255,0.15);border:none;color:#fff;
                    width:30px;height:30px;border-radius:50%;cursor:pointer;font-size:16px;
                    display:flex;align-items:center;justify-content:center;">✕</button>
            </div>
            <div style="padding:22px 24px 20px;">
                ${bodyHtml}
                <button id="rx-info-ok" style="
                    margin-top:18px;width:100%;background:linear-gradient(135deg,${gradFrom},${gradTo});
    ...
    overlay.querySelector('#rx-info-close').addEventListener('click', close);
    overlay.querySelector('#rx-info-ok').addEventListener('click', close);
...
async function renewPrescriptionsFromEpisode(sourceEpisode, targetEpisode, options = {}) {
    ...
    try {
        ...
        if (pushMeds && allMedicines.length > 0) {
            ...
            if (options.useExistingPrescription) {
                // Try to reuse the latest prescription in the target episode
                const existingPrescRels = targetEpisode.relationships?.prescriptions?.data || [];
                if (existingPrescRels.length > 0) {
                    // Sort descending by numeric ID — highest ID = most recently created prescription
                    const sortedPrescRels = [...existingPrescRels].sort((a, b) => Number(b.id) - Number(a.id));
                    newPrescriptionId = sortedPrescRels[0].id;
                    showToast('Adding to existing prescription...', 'info');
                    console.log('[RenewRx] Using most recent existing prescription:', newPrescriptionId);
                } else {
                    // No prescription exists in the target episode — tell the doctor clearly
                    showLoading(false);
                    showRxNoPrescriptionDialog();
                    return;
                }
            } else {
                ...
                try {
                    ...
                    newPrescriptionId = prescriptionResult.data.id;
                    console.log('[RenewRx] Created prescription:', newPrescriptionId);
                } catch (prescErr) {
                    showLoading(false);
                    // 406 = no diagnosis in episode yet
                    if (prescErr.message?.includes('406') || prescErr.message?.includes('Diagnoses')) {
                        showRxNoDiagnosisDialog();
                    } else {
                        showToast(`Failed to create prescription: ${prescErr.message}`, 'error');
                    }
                    return;
                }
            }
        ...
        if (error.message?.includes('406') || error.message?.includes('Diagnoses')) {
            showRxNoDiagnosisDialog();
        } else {
...
function wireBundleQuickSelect(){const btn=document.getElementById('quickBundleBtn'); if(btn&&!btn._wired){btn.addEventListener('click',()=>{showBundlePickerModal();}); btn._wired=true;}}
...
function showBundlePickerModal(){
    ...
    // Get patient-specific bundles if a patient is loaded
    let patientBundles = [];
    let patientName = '';
    const currentPatientNic = window.currentPatient?.attributes?.national_identification || '';
    if (currentPatientNic && typeof window._pbGetBundlesForPatient === 'function') {
        patientBundles = window._pbGetBundlesForPatient(currentPatientNic);
        patientName = window.currentPatient?.attributes?.patient_name || 'Patient';
    }
    
    if (bundles.length === 0 && patientBundles.length === 0) {
        showToast('No bundles available. Create one first.', 'info');
        return;
    }
    
    let html = '';
    
    // Patient-specific bundles section
    if (patientBundles.length > 0) {
        html += `<div style="padding: 12px 20px 8px; font-size: 12px; font-weight: 700; color: #7c3aed; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid #ede9fe;">
            <svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor" style="vertical-align: -2px; margin-right: 4px;">
              <path d="M10 10a4 4 0 100-8 4 4 0 000 8zm-7 8a7 7 0 0114 0H3z"/>
            </svg>
            ${escapeHtml(patientName)}'s Bundles
        </div>`;
        html += `<div class="bundle-picker-menu">${patientBundles.map((b, idx) => `<button class="bundle-pick-item pb-pick" data-pb-idx="${idx}" style="border-left: 3px solid #7c3aed;"><strong>${escapeHtml(b.name)}</strong><span class="bundle-pick-count">${b.tests.length} tests</span></button>`).join('')}</div>`;
    }
    
    // General bundles section
    if (bundles.length > 0) {
        if (patientBundles.length > 0) {
            html += `<div style="padding: 12px 20px 8px; font-size: 12px; font-weight: 700; color: #2563eb; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid #dbeafe;">General Bundles</div>`;
        }
        html += `<div class="bundle-picker-menu">${bundles.map((b, idx) => `<button class="bundle-pick-item" data-bundle-idx="${idx}"><strong>${escapeHtml(b.name)}</strong><span class="bundle-pick-count">${b.tests.length} tests</span></button>`).join('')}</div>`;
    }
    ...
    function applyTests(testsToApply, bundleName) {
        ...
        document.body.removeChild(overlay);
        showToast(`Applied bundle: ${bundleName} (${testsToApply.length} tests)`, 'success');
        setTimeout(() => { window._applyingBundle = false; }, 500);
    }

    // Patient-specific bundle clicks
    overlay.querySelectorAll('.pb-pick').forEach(btn => {
        btn.addEventListener('click', () => {
            const idx = parseInt(btn.getAttribute('data-pb-idx'), 10);
            const b = patientBundles[idx];
            if (!b) return;
            applyTests(b.tests.map(t => ({ code: t.code || '', asnd: t.asnd || '', name: t.name || '', vinaviServiceId: t.vinaviServiceId || null })), b.name);
        });
    });
    
    // General bundle clicks
    overlay.querySelectorAll('.bundle-pick-item:not(.pb-pick)').forEach(btn => {
        btn.addEventListener('click', () => {
            const idx = parseInt(btn.getAttribute('data-bundle-idx'), 10);
            const bundle = bundles[idx];
            if (!bundle) return;
            applyTests(bundle.tests.map(t => ({ code: t.code || '', asnd: t.asnd || '', name: t.name || '', vinaviServiceId: t.vinaviServiceId || null })), bundle.name);
        });
    });
}
...
function openSetBuilder(setId = null) {
    ...
    if (setId) {
        ...
        // Patient bundle edit mode: open builder directly with prefilled labs/name
        const pb = window._creatingPatientBundle;
        if (pb && pb.editingBundleId) {
            window._ignoreSetBuilderIframeUpdates = true;

            const prefillLabs = Array.isArray(pb.prefillLabs) ? pb.prefillLabs : [];
            currentSetData = {
                name: pb.editingBundleName || 'Patient Bundle',
                labs: prefillLabs.map(t => ({
                    code: t.code || '',
                    asnd: t.asnd || '',
                    name: t.name || '',
            ...
            if (titleEl) titleEl.textContent = 'Edit Patient Bundle';
            document.getElementById('setNameInput').value = currentSetData.name || '';
            updateSetBuilderDisplay();

            // Hide medications and advice tabs for patient bundles
            const tabs = document.querySelectorAll('.set-tab[data-tab="medications"], .set-tab[data-tab="advice"]');
...
function showSetNameWizard() {
    // Check if creating patient bundle
    const isPatientBundle = !!window._creatingPatientBundle;
    const patientName = isPatientBundle ? window._creatingPatientBundle.patientName : '';
    ...
    overlay.innerHTML = `
        <div class="bundle-picker-card" style="max-width: 520px;">
            <div class="bundle-picker-header">
                <h3>${isPatientBundle ? '👤 Create Patient Bundle' : '📋 Create New Clinical Bundle'}</h3>
                <button class="bundle-picker-close" aria-label="Close">&times;</button>
            </div>
            <div style="padding: 32px;">
                ${isPatientBundle ? `<p style="color: #2563eb; margin-bottom: 16px; font-size: 14px; font-weight: 600; background: #eff6ff; padding: 12px; border-radius: 6px; border-left: 3px solid #3b82f6;">👤 For: ${escapeHtml(patientName)}</p>` : ''}
                <p style="color: #6b7280; margin-bottom: 24px; font-size: 14px;">${isPatientBundle ? 'This bundle will only appear for this specific patient.' : 'First, let\'s give your clinical bundle a descriptive name. This will help you identify it later.'}</p>
    ...
    const handleContinue = () => {
        ...
        const isPatientBundle = !!window._creatingPatientBundle;
        if (titleEl) titleEl.textContent = isPatientBundle ? 'Create Patient Bundle' : 'Create Clinical Bundle';
    ...
    const handleClose = () => {
        if (window._creatingPatientBundle) {
...
function closeSetBuilder() {
    ...
    // Reset tab visibility (show all tabs)
    const tabs = document.querySelectorAll('.set-tab[data-tab="medications"], .set-tab[data-tab="advice"]');
    tabs.forEach(tab => tab.style.display = '');
    
    // Clear patient bundle flag if set
    if (window._creatingPatientBundle) {
        delete window._creatingPatientBundle;
    }
}
...
function saveCurrentSet() {
    ...
    try {
        // Check if we're creating a patient bundle
        if (window._creatingPatientBundle && typeof window._savePatientBundle === 'function') {
            // Save as patient bundle
            const saved = window._savePatientBundle(name, currentSetData.labs);
            if (saved) {
                showToast('Patient bundle created successfully', 'success');
                closeSetBuilder();
            } else {
                showToast('Failed to save patient bundle', 'error');
            }
        } else {
            // Normal bundle save
...
async function initializeDashboard() {
    ...
    // Initialize navigation early so Quick Links can work without Vinavi portal login
    try { initializeNavigation(); } catch (_) {}
    try { wireFailLogControls(); } catch (_) {}

    // Check authentication
    const userData = await checkAuthentication();
    
    if (!userData) {
        console.log('User not authenticated - enabling restricted mode');
        setRestrictedModeUI(true);
        try { window._activateView?.('quickLinks'); } catch (_) {}
        return;
    }

    // Full mode
    setRestrictedModeUI(false);
    ...
    // Listen for catalog messages even when no episode is loaded yet.
    // This is required for the Set Builder (patient bundles) fullscreen button.
    if (!window._catalogMessageListenerWired) {
        window.addEventListener('message', handleCatalogMessage);
        window._catalogMessageListenerWired = true;
    }
